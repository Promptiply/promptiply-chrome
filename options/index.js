...
(function () {
  ...
  const $anthropicModelCustom = document.getElementById(
  ...
  const $refineHotkeyRecording = document.getElementById(
    "refine-hotkey-recording"
  );
  const $saveSettings = document.getElementById("save-settings");
  const $saveProvidersSettings = document.getElementById(
  ...
  const $wizardSteps = Array.from(
    document.querySelectorAll("#profile-modal .step")
  );

  const $tabs = Array.from(document.querySelectorAll(".tab"));
  const $tabPanels = {
    general: document.getElementById("tab-general"),
    providers: document.getElementById("tab-providers"), // Fixed missing parenthesis
  ...
  const $onboardingSteps = Array.from(
    document.querySelectorAll("#onboarding-modal .step")
  );
  ...
  function stopRecordingHotkey() {
    ...
    if ($refineHotkeyDisplay)
      $refineHotkeyDisplay.classList.remove("recording");
    updateHotkeyDisplay();
    if (window._hotkeyRecorder) {
      window.removeEventListener(
        "keydown",
        window._hotkeyRecorder.keyDownHandler,
        true
      );
      window.removeEventListener(
        "keyup",
        window._hotkeyRecorder.keyUpHandler,
        true
      );
  ...
  function renderProfiles(p) {
    ...
    p.list.forEach((prof) => {
      ...
      line.textContent = [prof.persona, prof.tone]
        .filter(Boolean)
        .join(" ΓÇó ");
      ...
      activate.textContent =
        p.activeProfileId === prof.id ? "Active" : "Set Active";
      ...
      del.addEventListener("click", () => {
        ...
        if (updated.activeProfileId === prof.id)
          updated.activeProfileId = updated.list[0]?.id || null;
        chrome.storage.sync.set({ [STORAGE_PROFILES]: updated }, () =>
          renderProfiles(updated)
        );
      });
  ...
  function renderPredefinedProfiles() {
    ...
    PREDEFINED_PROFILES.forEach((p) => {
      ...
      importBtn.addEventListener("click", () =>
        importPredefinedProfile(p, { activate: false })
      );
    ...
    const importAll = document.getElementById("import-all-predefined");
    if (importAll) {
      importAll.addEventListener("click", importAllPredefined);
    }
  }
  ...
  function setWizardStep(step) {
    ...
    if (wizardState.step === 1) {
      ...
      const nameField = document.createElement("div");
      nameField.className = "field";
      nameField.innerHTML = '<label>Name</label><input type="text" disabled />';
      $wizardBody.appendChild(nameField);

      const toneField = document.createElement("div");
      toneField.className = "field";
      toneField.innerHTML = '<label>Tone</label><input type="text" disabled />';
      $wizardBody.appendChild(toneField);
    }
  }
  ...
  $wizardNext?.addEventListener("click", () => {
    ...
    if (wizardState.step === 1) {
      ...
      wizardState.persona = (
        document.getElementById("w-persona")?.value || ""
      ).trim();
      wizardState.tone = (
        document.getElementById("w-tone")?.value || ""
      ).trim();
    } else if (wizardState.step === 2) {
      wizardState.guidelines = (
        document.getElementById("w-guidelines")?.value || ""
      )
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean);
  ...
  function openOnboardingWizard() {
    ...
    setOnboardingStep(1);
    try { showToast('Onboarding opened'); } catch (_) {}
  }
  ...
  function setOnboardingStep(step) {
    ...
    try {
      $onboardingSteps.forEach((s) =>
        s.classList.toggle(
          "active",
          Number(s.dataset.step) === onboardingState.step
        )
      );
      ...
      $onboardingNext?.classList.toggle(
        "tab-panel-hidden",
        onboardingState.step === 4
      );
      $onboardingFinish?.classList.toggle(
        "wizard-save-hidden",
        onboardingState.step !== 4
      );
    } catch (e) {
    ...
    try {
      if (onboardingState.step === 1)
        safeRender("renderModesStep", renderModesStep);
      else if (onboardingState.step === 2)
        safeRender("renderSetupStep", renderSetupStep);
      else if (onboardingState.step === 3)
        safeRender("renderProfileStep", renderProfileStep);
      else safeRender("renderSuccessStep", renderSuccessStep);
    } catch (e) {
      logOnboardingError(e, "setOnboardingStep", step);
    }
  }
  ...
  function safeRender(name, fn) {
    try {
      ...
      if (!$onboardingBody) {
        ...
        if (!$onboardingBody) {
          try {
            ...
            /* ignore fallback create failure */
          }
        }
      }
      return fn();
      ...
      try {
        logOnboardingError(e, name, onboardingState && onboardingState.step);
      } catch (_) {}
    }
  }
  ...
  function renderModesStep() {
    ...
    <div class="mode-card ${
      onboardingState.selectedMode === "api" ? "selected" : ""
    }" data-mode="api">
      <h3>API Mode</h3>
    ...
    <div class="mode-card ${
      onboardingState.selectedMode === "webui" ? "selected" : ""
    }" data-mode="webui">
      <h3>WebUI Mode</h3>
    ...
    <div class="mode-card ${
      onboardingState.selectedMode === "local" ? "selected" : ""
    }" data-mode="local">
  ...
  `;
      ...
      Array.from($onboardingBody.querySelectorAll(".mode-card")).forEach(
        (card) => {
      ...
      if (onboardingState.selectedMode === "api") {
        ...
        <div class="field">
          ...
          <select id="ob-provider">
            <option value="openai" ${
              onboardingState.selectedProvider === "openai" ? "selected" : ""
            }>OpenAI (ChatGPT)</option>
            <option value="anthropic" ${
              onboardingState.selectedProvider === "anthropic" ? "selected" : ""
            }>Anthropic (Claude)</option>
          </select>
        </div>
  <div id="ob-openai-fields" class="${
    onboardingState.selectedProvider === "openai" ? "" : "hidden"
  }">
          <div class="field">
            <label>OpenAI API Key</label>
            <input id="ob-openai-key" type="password" placeholder="sk-..." value="${escapeHtml(
  ...
  <div id="ob-anthropic-fields" class="${
    onboardingState.selectedProvider === "anthropic" ? "" : "hidden"
  }">
          <div class="field">
            <label>Anthropic API Key</label>
            <input id="ob-anthropic-key" type="password" placeholder="sk-ant-..." value="${escapeHtml(
              onboardingState.apiKey
            )}" />
          </div>
          <div class="field">
  ...
  <p class="muted muted-top-margin muted-small">You can skip this step and set up your API key later in Settings.</p>
      `;
        ...
        const $obAnthropicFields = document.getElementById(
          "ob-anthropic-fields"
        );
        ...
        if ($obProvider) {
          ...
          $obOpenaiFields.classList.toggle(
            "hidden",
            $obProvider.value !== "openai"
          );
          $obAnthropicFields.classList.toggle(
            "hidden",
            $obProvider.value !== "anthropic"
          );
          ...
          $obProvider.addEventListener("change", () => {
            ...
            $obAnthropicFields.classList.toggle(
              "hidden",
              onboardingState.selectedProvider !== "anthropic"
            );
          });
          // Pre-select model options if onboardingState.model was set
          const $obOpenaiModel = document.getElementById("ob-openai-model");
          const $obAnthropicModel =
            document.getElementById("ob-anthropic-model");
          if ($obOpenaiModel && onboardingState.model) {
            Array.from($obOpenaiModel.options).forEach(
              (o) => (o.selected = o.value === onboardingState.model)
            );
          }
          if ($obAnthropicModel && onboardingState.model) {
            Array.from($obAnthropicModel.options).forEach(
              (o) => (o.selected = o.value === onboardingState.model)
            );
          }
      ...
      function renderProfileStep() {
        console.log("[promptiply] Debug: Entering renderProfileStep");
        return safeRender("renderProfileStep", function () {
          if (!$onboardingBody) {
            console.warn("[promptiply] Debug: $onboardingBody is null");
            return;
          }
          console.log("[promptiply] Debug: Rendering profile step");
          $onboardingBody.innerHTML = `
    <div class="onboarding-section">
    ...
    <div class="field">
      ...
      <input id="ob-profile-name" type="text" placeholder="e.g., Technical Tutor" value="${escapeHtml(
    ...
    <div class="grid">
      <div class="field">
        ...
        <input id="ob-profile-persona" type="text" placeholder="e.g., Senior AI Engineer" value="${escapeHtml(
          onboardingState.profilePersona
        )}" />
      </div>
      <div class="field">
        <label>Tone (optional)</label>
        <input id="ob-profile-tone" type="text" placeholder="e.g., concise, friendly" value="${escapeHtml(
          onboardingState.profileTone
        )}" />
      </div>
    </div>
  ...
  `;
          console.log("[promptiply] Debug: Profile step rendered");
        });
      }

      function renderSuccessStep() {
        return safeRender("renderSuccessStep", function () {
          if (!$onboardingBody) return;
          $onboardingBody.innerHTML = `
    <div class="onboarding-success">
      ...
      <div class="onboarding-summary">
        <div class="mb-12"><strong>Mode:</strong> ${
          onboardingState.selectedMode === "api"
            ? "API"
            : onboardingState.selectedMode === "webui"
            ? "WebUI"
            : "Local (Llama 3)"
        }</div>
        ${
          onboardingState.selectedMode === "api" && onboardingState.apiKey
            ? `<div class="mb-12"><strong>Provider:</strong> ${
                onboardingState.selectedProvider === "openai"
                  ? "OpenAI"
                  : "Anthropic"
              }</div>`
        ...
        ${
          onboardingState.profileName
            ? `<div class="mb-12"><strong>Profile:</strong> ${escapeHtml(
                onboardingState.profileName
              )}</div>`
            : ""
        }
  ...
  `;
      ...
      function handleOnboardingNext() {
        if (
          ...
          if (onboardingState.selectedProvider === "openai") {
            ...
            const $obOpenaiModel = document.getElementById("ob-openai-model");
            if ($obOpenaiKey)
              onboardingState.apiKey = $obOpenaiKey.value.trim();
            ...
            const $obAnthropicModel =
              document.getElementById("ob-anthropic-model");
            if ($obAnthropicKey)
              onboardingState.apiKey = $obAnthropicKey.value.trim();
            if ($obAnthropicModel)
              onboardingState.model = $obAnthropicModel.value;
          }
        } else if (onboardingState.step === 3) {
          const $obProfileName = document.getElementById("ob-profile-name");
          const $obProfilePersona =
            document.getElementById("ob-profile-persona");
          const $obProfileTone = document.getElementById("ob-profile-tone");
          if ($obProfileName)
            onboardingState.profileName = $obProfileName.value.trim();
          if ($obProfilePersona)
            onboardingState.profilePersona = $obProfilePersona.value.trim();
          if ($obProfileTone)
            onboardingState.profileTone = $obProfileTone.value.trim();
        }
      ...
      function handleOnboardingNextWrapper() {
        if (onboardingState.step === 1) {
          if (!onboardingState.selectedMode) {
            // show temporary hint
            const hint = document.createElement("div");
            hint.className = "onboarding-hint";
            hint.textContent = "Please choose a mode to continue.";
            const container = $onboardingBody.querySelector(
              ".onboarding-section"
            );
            if (container) {
              const existing = container.querySelector(".onboarding-hint");
              if (!existing) container.appendChild(hint);
              setTimeout(() => {
                try {
                  hint.remove();
                } catch (_) {}
      ...
      if ($onboardingNext) {
        $onboardingNext.removeEventListener("click", handleOnboardingNext);
      ...
      function finishOnboarding() {
            ...
            if (onboardingState.apiKey) {
              if (onboardingState.selectedProvider === "openai") {
                ...
                settings.anthropicModel =
                  onboardingState.model || "claude-haiku-4-5";
              }
            }
          }
        ...
        if (onboardingState.profileName) {
          chrome.storage.sync.get([STORAGE_PROFILES], (data) => {
            const profiles = data[STORAGE_PROFILES] || {
              list: [],
      ...
      (function bindRunOnboarding() {
        function attach() {
          ...
          const wrapper = function () {
            try {
              if (typeof openOnboardingWizard === "function")
                return openOnboardingWizard();
              if (typeof window.startOnboarding === "function")
                return window.startOnboarding();
              ...
              if (typeof renderModesStep === "function")
                return renderModesStep();
            } catch (e) {
              console.warn("[promptiply] run-onboarding wrapper failed", e);
            }
          };
          btn.addEventListener("click", wrapper);
          try {
            if (btn.dataset) btn.dataset.prOnboardingAttached = "1";
          } catch (_) {}
          return true;
        }
        ...
        try {
          if (!document._prOnboardingDelegated) {
              ...
              (ev) => {
                try {
                  const t =
                    ev.target &&
                    ev.target.closest &&
                    ev.target.closest("#run-onboarding");
                  if (t) {
                    ...
                    console.log(
                      "[promptiply] delegated click detected for run-onboarding"
                    );
                    ...
                    const wrapperCall = function () {
                      try {
                        ...
                        const m = document.getElementById("onboarding-modal");
                        const obody =
                          document.getElementById("onboarding-body");
                        if (m) m.classList.add("modal-show");
                        if (
                          obody &&
                          (obody.innerHTML || "").trim().length === 0
                        ) {
                      ...
                      <div class="mode-card ${
                        onboardingState.selectedMode === "webui"
                          ? "selected"
                          : ""
                      }" data-mode="webui" tabindex="0">
                        <h3>WebUI Mode</h3>
                        <p>Opens a background tab to ChatGPT or Claude, sends your prompt, and reads the response. No API key needed.</p>
                        <span class="mode-badge badge-simple">Simple Setup</span>
                      </div>
                      <div class="mode-card ${
                        onboardingState.selectedMode === "local"
                          ? "selected"
                          : ""
                      }" data-mode="local" tabindex="0">
                        <h3>Local Mode (Llama 3)</h3>
                        <p>Runs entirely in your browser using Llama 3.1 8B model. Completely private and offline after initial download (~5GB).</p>
                        <span class="mode-badge badge-private">100% Private</span>
                      </div>
                      `;
                        ...
                        console.warn(
                          "[promptiply] delegated run-onboarding failed",
                          e
                        );
                      }
                    };
            ...
            try {
              document._prOnboardingDelegated = true;
            } catch (_) {}
          }
        } catch (_) {
          /* ignore */
      ...
      try {
        window.openOnboardingWizard = function () {
          try {
            ...
            console.warn("openOnboardingWizard failed", e);
          }
        };
      } catch (_) {}
      try {
        window.setOnboardingStep = function (s) {
          try {
            return setOnboardingStep(s);
          } catch (e) {
            console.warn("setOnboardingStep failed", e);
          }
        };
      } catch (_) {}
      try {
        ...
        document.addEventListener("DOMContentLoaded", () => {
          try {
            ...
            if (duplicates.length) {
              ...
              banner.textContent = `Duplicate DOM IDs detected: ${duplicates
                .map((d) => d[0] + "(" + d[1] + ")")
                .join(", ")}`;
      ...
      // Ensure core UI buttons and tabs are bound even if script ran before DOM
        ...
        try {
          ...
          if (runBtn && !runBtn.dataset.prAttached) {
            runBtn.addEventListener('click', () => {
              try { openOnboardingWizard(); } catch (e) { console.error('openOnboardingWizard failed', e); }
            });
            try { runBtn.dataset.prAttached = '1'; } catch (_) {}
          ...
          if (saveBtn && !saveBtn.dataset.prAttached) {
            saveBtn.addEventListener('click', () => {
              try { saveSettings(); } catch (e) { console.error('saveSettings failed', e); }
            });
            try { saveBtn.dataset.prAttached = '1'; } catch (_) {}
          ...
          if (saveProvBtn && !saveProvBtn.dataset.prAttached) {
            saveProvBtn.addEventListener('click', () => {
              try { saveSettings(); } catch (e) { console.error('saveProvidersSettings failed', e); }
            });
            try { saveProvBtn.dataset.prAttached = '1'; } catch (_) {}
          ...
          if (newProfileBtn && !newProfileBtn.dataset.prAttached) {
            newProfileBtn.addEventListener('click', () => {
              try { openWizard(); } catch (e) { console.error('openWizard failed', e); }
            });
            try { newProfileBtn.dataset.prAttached = '1'; } catch (_) {}
          ...
          if (tabEls.length) {
            tabEls.forEach((t) => {
              if (!t.dataset.prAttached) {
                t.addEventListener('click', () => selectTab(t.dataset.tab));
                try { t.dataset.prAttached = '1'; } catch (_) {}
                attachedAny = true;
              }
            });
            if (attachedAny) console.log('[promptiply] attachCoreListeners: bound tab elements');
          }

          // Success when we've attached at least one handler AND there are tab elements available
      ...
      // Small visual toast helper for feedback during testing
      ...
      // Try immediate attach, then DOMContentLoaded, then MutationObserver fallback
      ...
      // Delegated click handler as a last-resort fallback to ensure clicks land
      document.addEventListener('click', (ev) => {
        try {
          const t = ev.target && ev.target.closest && ev.target.closest('.tab');
          if (t && t.dataset && t.dataset.tab) {
            console.log('[promptiply] delegated tab click ->', t.dataset.tab);
            selectTab(t.dataset.tab);
            return;
          }
          const run = ev.target && ev.target.closest && ev.target.closest('#run-onboarding');
          if (run) {
            console.log('[promptiply] delegated click: run-onboarding');
            try { openOnboardingWizard(); showToast('Onboarding opened'); } catch (e) { console.error(e); }
            return;
          }
          const save = ev.target && ev.target.closest && ev.target.closest('#save-settings');
          if (save) {
            console.log('[promptiply] delegated click: save-settings');
            try { saveSettings(); showToast('Settings saved'); } catch (e) { console.error(e); }
            return;
          }
          const saveProv = ev.target && ev.target.closest && ev.target.closest('#save-providers-settings');
          if (saveProv) {
            console.log('[promptiply] delegated click: save-providers-settings');
            try { saveSettings(); showToast('Provider settings saved'); } catch (e) { console.error(e); }
            return;
          }
          const newProf = ev.target && ev.target.closest && ev.target.closest('#new-profile');
          if (newProf) {
            console.log('[promptiply] delegated click: new-profile');
            try { openWizard(); } catch (e) { console.error(e); }
            return;
          }
        } catch (e) {
          /* ignore */
      ...
      console.log("[promptiply] Debug: index.js execution completed");

      // Add detailed logs to debug runtime issues
      console.log("[promptiply] Debug: Script execution started");

      // Verify button bindings
      const buttonsToCheck = [
        { id: "run-onboarding", name: "Run Onboarding Wizard" },
        { id: "save-settings", name: "Save Settings" },
        { id: "save-providers-settings", name: "Save Providers Settings" },
        { id: "new-profile", name: "New Profile" },
      ];

      buttonsToCheck.forEach(({ id, name }) => {
        const button = document.getElementById(id);
        if (button) {
          console.log(`[promptiply] Debug: Button found - ${name}`);
          button.addEventListener("click", () => {
            console.log(`[promptiply] Debug: ${name} button clicked`);
          });
        } else {
          console.warn(`[promptiply] Debug: Button not found - ${name}`);
        }
      });

      console.log("[promptiply] Debug: Script execution completed");

      console.log("[promptiply] Debug: index.js fully loaded and closed");
    });
  }
});
